...
public struct ParsedDate: Sendable, Equatable {
    ...
    /// True when the caller wrote only a day. `create_event` uses this to decide
    /// all-day: there is no separate flag, because two sources of truth for the same
    /// fact drift apart.
...
/// it as 8 October for a caller who meant 10 August, and a calendar server that silently
/// moves an appointment by two months is worse than one that refuses the input.
public enum DateParsing {
    ...
    /// `2026-08-12T09:00` — the form `create_event` accepts, so a delete can print a
    /// recreate call that actually parses.
    public static func roundTrip(_ date: Date, isAllDay: Bool, calendar: Calendar) -> String {
        let parts = calendar.dateComponents([.year, .month, .day, .hour, .minute], from: date)
        let day = String(
            format: "%04d-%02d-%02d", parts.year ?? 0, parts.month ?? 0, parts.day ?? 0)
        guard !isAllDay else { return day }
